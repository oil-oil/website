...
author: "Garry Chen"
href: "https://github.com/neaped"
avatar: "https://avatars.githubusercontent.com/u/3713305?v=4"
date: "2021/11/22"
keywords: Kubernetes,Apache APISIX Ingress,Nocalhost,部署开发
...
Nocalhost 将使 `apisix-ingress-controller` 结束开发模式, 并重置 `apisix-ingress-controller` 到其原始版本。启用端口转发来看看退出开发模式后的结果。
...
- Nocalhost 将会先让 `apisix-ingress-controller` 进入开发模式，并运行在 [`dev config`](https://nocalhost.dev/zh-CN/docs/config/config-develop) 定义的调试命令
...
- Nocalhost 将会先让 `apisix-ingress-controller` 进入开发模式，并运行在 [`dev config`](https://nocalhost.dev/zh-CN/docs/config/config-develop) 定义的运行命令
