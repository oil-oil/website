...
之所以我们要做 NGINX 和 Kong 的替代项目，实际和我们后端架构演变史大背景息息相关，我会先和大家一起分享后端架构演变过程，这非常重要。以此为背景，我们会发现 NGINX 和 Kong 一些比较严重的问题。反过来说如果他们没有问题，APISIX 也不可能有机会，我们公司也更没有发展机会，最后也会和大家聊聊 APISIX 期望的未来。
...
2019 年我与合伙人温铭一起创办了深圳支流科技公司，它是一家以开源为依托的商业化公司。这家公司承载了我俩很多个人理想，也可以说是在做每一位普通程序员的理想，不想庸庸碌碌 996，我经常对别人说我的梦想就是“把我的名字刻入史册”，悲催的是人类已经不需要史册了。作为一家开源创业公司，我们有两个非常有代表的标签，Linux 和 Apache 开源基金会。

![20210625004](../static/images/20210625004.png)

这是我们团队，大家主要是远程协作，所有人聚在一起比较难。公司早期阶段只有五六个人时，还能比较容易的把团队聚起来，但从今年之后就一直没聚齐过，这是我们今年到目前以来最齐的一次（但依然有几位同学没能一起），中美远程协作的我们总共 20+ 人，但却分布在中国 9 个不同城市。
...
我们再看看左侧比较有意思的 JAVA ，Spring Cloud 内置 API 网关先后经历了 ZUUL、ZUUL2，但还是不好用，性能、架构官方都不满意，所以 Spring Cloud 官方发起了新项目 Spring Cloud Gateway，最终形成全家桶解决方案。但它既不支持动态，性能又不行，跟右上角这部分 C 语言体系上的产品相比差距甚大。

最后说说右下角部分的服务网格，对于服务网格已经形成一种政治正确的选择就是 istio(CP) + envoy(DP)。对于 envoy 最大问题在于自身臃肿以及 C++ 这门语言自身的高复杂度，所以我们又看到了阿里巴巴开源的 mosn，一句话概括：Golang 版本的 envoy。
...
我们继续一起聊一下 Kong，2019 年我开始写 APISIX 时候，当时的目标比较直就是超越 Kong，我们花了很多的时间研究它和其他 API 网关产品。在经过基本调研后我们找到了它的一些明显弱点：技术选型缺陷、遍历的方式做路由匹配以及自身代码臃肿。
...
这张图的左右分别是 DP（Date Plane）和 CP（Control Plane），跟大家所熟悉的后端服务体系一样。APISIX 从架构第一天就没有想去自己造新东西，所以关于配置中心选择了当下最成熟的 etcd。这让 APISIX 的开发成本、复杂度一下子降低了有很多，利用它几乎零开发的完成增量通知、历史回滚、高可用等重要功能刚需。

在这个架构里面，你找不到一个死点。这里的任何一个服务出现异常宕机等事故，都不会影响 APISIX 正常对外提供服务的能力。当整体架构中的每一个点都支持高可用时，用户生产系统的高稳定性就非常容易实现。

![20210625012](../static/images/20210625012.png)

这是 APISIX 的生态图，从该图可以准确看到目前都支持了哪些周边生态。左侧是支持的协议，可以看到常见的 7 层协议有 HTTP(S)、HTTP2、Dubbo、QUIC 和物联网协议 MQTT 等，4 层协议有 TCP/UDP 。右侧部分则是一些开源或者 SaaS 服务，比如 SkyWalking、Prometheus 、Vault 等。下面就是一些比较常见的操作系统环境、云厂商和硬件环境，作为一家中国企业，我们自然也支持 ARM64、麒麟等一些国产系统和硬件平台。

![20210625013](../static/images/20210625013.png)

给大家简单地汇报一下 APISIX 当前状态，APISIX 从开源到现在不到两年的时间，APISIX 已经成为了全世界最活跃的开源 API 网关项目，而且这个状态已经持续了一年多。请记住最下面一句话，APISIX 已经**生产可用，功能、性能、架构全面优于 Kong。**在 2019 年 9 月份贝壳找房就已经把 APISIX 项目用到生产环境，目前日请求次数已超过十个亿。
...
![20210625015](../static/images/20210625015.png)
...
举个例子比如 Kubernetes Ingress、APISIX Ingress 两个项目，当只有一条路由时跑压测，你会发现 Kubernetes Ingress 的性能只有 APISIX Ingress 的一半，后者性能更好让人觉得不太可能。

最后说说用户量，通过第三方统计网站收集，目前 APISIX 在公网部署了几千节点，最大用户是微博日请求量在 80~100 亿次。

![20210625016](../static/images/20210625016.png)

APISIX 支持多语言的特性，已经放到开源项目，欢迎感兴趣的同学可以随时关注并参与。这个实现方案的优势是简单、通用，大家可以原生的使用自己熟悉语言。稍微展望一下未来，尤其是我刚提到的 ZUUL、ZUUL2 或者是 Spring Cloud Gateway 这样的用户，为了方便迁移，我们会在类底层做适配，代码无感知的方式过渡到 APISIX 方式，进一步降低迁移成本。
...
高性能、动态、社区活跃是 APISIX 的王牌，它们已经形成了良性互动。这些王牌中的每一张，背后有很高含金量。感谢这次创业，它把我们逼在角落要给世界最优秀的解决方案，不辱使命我和伙伴们做到了 ^_^。
...
我先跟大家说目前完成度给大家心里打个底。目前右上角的  SLB、Gateway、Ingress 已经全部开源，无论开源还是商业用户，目前这三个点都有真实用户覆盖。左侧的 Spring Cloud Gateway 场景，目前也有开源用户用于生产。右下角的服务网格，欢迎大家关注，我们将会在今年 Q2 季度开源（注：截止目前已经开源，[点击查看](https://mp.weixin.qq.com/s/t7oga6xP2JpdcraixwSwVg)）。
...
目前官方 Kubernetes Ingress Controller 项目的主要维护作者退休，项目更新迭代受到很大影响，此外由于它是基于 NGINX 配置文件方式，也天然的继承了 NGINX 自身的劣势，比如静态路由、路由能力弱等。APISIX Ingress Controller 则完整解决了上面提到的所有问题，继承了 APISIX 的所有优势，此外还支持原生 k8s CRD ，方便用户迁移。

![20210625022](../static/images/20210625022.png)

服务网格，这里面有必要跟大家重点聊聊。未来五年或者十年之后，最有可能主流的服务端架构是什么？如果让我回答，我选择服务网格。虽然眼下的它，还有各种各样的问题。如果我们重头开始服务网格，无异于以卵击石。所以我们经过市场调研和技术分析后，最后确定让 APISIX 可以支持 xDS 协议，这样 APISIX Mesh 就诞生了。对于终端用户来说，具体使用的 Envoy 还是 APISIX ，可以根据自己需要自由选择。最后的比较就变成了 APISIX vs Envoy ，很明显前者在开发难度、性能等方面，具有非常大的优势。

![20210625023](../static/images/20210625023.png)

右图部分就是 APISIX Mesh 的内部架构图。题外话，该项目主要负责人是我司超哥(张超)，因为他个人能力突出我甚是佩服，虽然我比他大十几岁，但我管他叫超哥 ^_^。

![20210625024](../static/images/20210625024.png)

聊了这么多 APISIX 的当下，也和大家聊聊 APISIX 的未来。

因为 APISIX 目前是 Apache 基金会项目，所以它已经不再属于个人或公司，而是全人类共享财产。这样社区中的每一个你，都有权利决定他往哪个方向发展。下面这几项是我司在 APISIX 项目上的一些初步安排，前面已经提及部分就不再敖述，重点聊一下多配置中心和自定义 RPC 协议。

开源版本 APISIX 目前默认搭配的配置中心是 etcd ，虽然目前它依旧是最好的选择，但我们在和用户沟通时依然经常会听到是否支持其他配置中心，比较常见的原因是 etcd 这个产品太新了，公司现有运维产品支持列表中没有它。所以我们计划让 APISIX 可以与其他配置中心协作。如果你对这个需求比较感兴趣，可以随时联系我司。自定义 RPC 协议，这个是我个人想做的功能点，不过我的时间不一定可控（公司事情比较多），暂定今年年底之前给大家开源。

![20210625025](../static/images/20210625025.png)
